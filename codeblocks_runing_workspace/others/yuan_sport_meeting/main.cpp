...
void enter_result()
{
	...
	do
	{
		...
		{
			printf("第%d运动员的成绩:", i + 1);
			scanf("%f", &score);
			for (int j = 0; j < 3; j++)
			{
				if (playerArr[i].pro[j].code == pro[choice-1].code)
				{
					playerArr[i].pro[j].score = score;
				}
			}
		}
		...
		for(int i=0;i<count;i++)
        {
            printf("第%d名%s运动员的成绩为：\n",i+1,playerArr[i].playerName);
            for (int j = 0; j < 3; j++)
			{
				if (playerArr[i].pro[j].code == pro[choice-1].code)
				{
					printf("该项成绩为%f\n",playerArr[i].pro[j].score);
				}
			}

        }
		printf("退出？YES(1)/NO(0)");
		scanf("%d", &is_exit);
	} while (!is_exit);
		...
		if (p.playerCode == player_code)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (strcmp("无",p.pro[i].name)!=0)
				{
					...
					while ((fread(&q, sizeof(player), 1, fp1))!=NULL)
					{
						if (q.playerCode == player_code)
						{
							printf("%8s\t%8f\n", q.pro[i].name, q.pro[i].score);
						}
					}
					fclose(fp1);
				}
