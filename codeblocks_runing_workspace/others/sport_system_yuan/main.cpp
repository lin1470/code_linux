...
int check_account()
{
    char real_account[MAX_LENTH];
    char real_password[MAX_LENTH];
    int i;
    fflush(stdin);
    printf("account:");
    gets(real_account);
    printf("password:");
    for(i=0;i<MAX_LENTH&&((real_password[i]=getch())!=13);i++)
 	   		    putchar('*');
 	   		    putchar('\n');
 		real_password[i]='\0';
...
void modifyProject()
{
	...
	switch (modifyCode)
	{
		case 1:
			...
			{
                strcpy(proArr[modifyProCode-1].projectName, modifyStrData);
			}
			break;
		case 2:
			if (didDeleteOrModify())
			{
				strcpy(proArr[modifyProCode-1].projectPlace, modifyStrData);
			}
			break;
		case 3:
			if (didDeleteOrModify())
		...
		case 5:
			if (didDeleteOrModify())
			{
				proArr[modifyProCode -1].timeOfGroupTake = modifyIntData;
			}
			break;
		case 6:
			if (didDeleteOrModify())
			{
				proArr[modifyProCode - 1].heighestRecord = modifyIntData;
			}
			break;
	default:
		break;
